...
.create-page(v-if="!success")
    h1 Введи данные
    .create-page__form
        NOInput.create-page__question(
            label="Вопрос"
            :value="questionText"
            @input-data="(value: string) => (questionText = value)"
        )
...
export default defineComponent({
    ...
    setup() {
        ...
        const createdQuestionLink: ComputedRef = computed(
            (): string => `https://neverniiotvet.ru/question/${question.value.id}`
        );
